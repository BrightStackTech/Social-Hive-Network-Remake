...
const getAllComposts = async (req, res) => {
  try {
    ...
    const composts = await ComPost.find({
      ...
      .populate({ path: 'comments', select: '_id', match: { commentedBy: { $nin: excludeIds } } });

    const formattedComposts = composts.map(c => {
      const obj = c.toObject();
      obj.commentCount = obj.comments ? obj.comments.length : 0;
      return obj;
    });

    res.status(200).json(formattedComposts);
  } catch (error) {
    res.status(400).json({ error: error.message });
  }
};
...
const getCompostById = async (req, res) => {
  try {
    ...
    const compost = await ComPost.findById(req.params.id)
      ...
      .populate({
        path: 'comments',
        match: { commentedBy: { $nin: excludeIds } },
        populate: {
          path: 'commentedBy',
          select: 'username',
        },
...
const createReply = async (req, res) => {
  try {
    const { replyBody, communityId, postId, parentId } = req.body;
    ...
    if (parentId) {
      // If parentId exists, it's a sub-reply
      const parentReply = await ComReply.findById(parentId);
      if (parentReply) {
        parentReply.replies.push(reply._id);
        await parentReply.save();
      }
    } else {
      // It's a top-level reply to a comment
...
const getRepliesByCommentId = async (req, res) => {
  try {
    ...
    const comment = await ComComment.findById(commentId).populate({
      path: 'replies',
      match: { repliedBy: { $nin: excludeIds } },
      populate: [
...
const getRepliesByReplyId = async (req, res) => {
  try {
    const { replyId } = req.params;
    const admins = await User.find({ $or: [{ isAdmin: true }, { isFreezed: true }] }).select('_id');
    const excludeIds = admins.map(a => a._id);

    const reply = await ComReply.findById(replyId).populate({
      path: 'replies',
      match: { repliedBy: { $nin: excludeIds } },
      populate: [
        { path: 'repliedBy', select: 'username' },
...
const deleteReply = async (req, res) => {
  try {
    ...
    // Recursive delete helper for replies
    const deleteNestedReplies = async (repliesArray) => {
      if (!repliesArray || repliesArray.length === 0) return;
      
      const childReplies = await ComReply.find({ _id: { $in: repliesArray } });
      for (const child of childReplies) {
        if (child.replies && child.replies.length > 0) {
          await deleteNestedReplies(child.replies);
        }
    ...
    if (reply.parentId) {
      await ComReply.findByIdAndUpdate(reply.parentId, {
    ...
    res.status(200).json({ message: 'Reply and sub-replies deleted successfully' });
  } catch (error) {
    res.status(400).json({ error: error.message });
  }
};
...
const getUserCommunitiesFeed = asyncHandler(async (req, res) => {
  ...
  const joinedPosts = await ComPost.find({
    ...
    author: { $nin: [userId, ...excludeIds] },
    upvotedBy: { $ne: userId },
    downvotedBy: { $ne: userId }
  })
    .populate('author', 'username')
    .populate('community', 'communityName')
    .populate({ path: 'comments', select: '_id', match: { commentedBy: { $nin: excludeIds } } })
  ...
  const formattedJoinedPosts = joinedPosts.map(c => {
    const obj = c.toObject();
    obj.commentCount = obj.comments ? obj.comments.length : 0;
    return obj;
  });
  ...
  const formattedUnjoinedPosts = unjoinedPosts.map(c => {
    const obj = c.toObject();
    obj.commentCount = obj.comments ? obj.comments.length : 0;
    return obj;
  });

  res.status(200).json({ joinedPosts: formattedJoinedPosts, unjoinedPosts: formattedUnjoinedPosts });
});
...
const searchComPosts = async (req, res) => {
  try {
    ...
    const formattedPosts = posts.map(c => {
      const obj = c.toObject();
      obj.commentCount = obj.comments ? obj.comments.length : 0;
      return obj;
    });
...
const getUserCommunityPosts = async (req, res) => {
  try {
    ...
    const posts = await ComPost.find({ author: userId })
      .populate('author', 'username profilePicture college')
      .populate('community', 'communityName admin')
      .sort({ createdAt: -1 });
...
export { createCompost, deleteCompostById, getAllComposts, getCompostById, 
  ...
  createReply, getRepliesByCommentId, getRepliesByReplyId, upvoteReply, downvoteReply, removeUpvoteReply,
