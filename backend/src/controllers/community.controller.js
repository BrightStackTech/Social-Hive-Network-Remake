...
const getCommunityByName = asyncHandler(async (req, res) => {
  ...
  const community = await Community.findOne({ communityName: communityName.toLowerCase() })
    ...
    .populate('joinedBy', 'username profilePicture bio college')
    .select('+removedMem +pendingReq'); 

  if (!community) {
    throw new ApiError(404, 'Community not found');
  }

  // Self-Healing Count Logic: Filter out deleted users from joinedBy
  community.joinedBy = community.joinedBy.filter(user => user !== null);
  const validCount = community.joinedBy.length;

  // If we found deleted users, synchronize the database
  if (validCount !== community.joinedCount) {
    community.joinedCount = validCount;
    await Community.findByIdAndUpdate(community._id, { 
      joinedBy: community.joinedBy.map(u => u._id),
      joinedCount: validCount 
    });
  }
...
const joinCommunity = asyncHandler(async (req, res) => {
  ...
  if (community.removedMem.includes(userId)) {
    throw new ApiError(403, 'You were removed from this community. Please send a join request instead.');
  }

  community.joinedBy.push(userId);
  
  // Recalculate joinedCount from the actual array to be safe
  community.joinedCount = community.joinedBy.length;
...
const handleJoinRequest = asyncHandler(async (req, res) => {
  ...
  if (action === 'approve') {
    community.joinedBy.push(userId);
    community.joinedCount = community.joinedBy.length;
    // Reset removal status if they were previously removed
    community.removedMem = community.removedMem.filter(id => id.toString() !== userId.toString());
...
const deleteCommunity = asyncHandler(async (req, res) => {
  ...
  const community = await Community.findOneAndDelete({ communityName: communityName.toLowerCase() });
...
const getUnjoinedCommunities = asyncHandler(async (req, res) => {
  ...
  for (const community of communities) {
    if (!community.admin) {
      // DELETE ORPHAN
      await ComPost.deleteMany({ community: community._id });
      await ComComment.deleteMany({ community: community._id });
  ...
  return res.status(200).json(new ApiResponse(200, validCommunities, 'Unjoined communities fetched successfully'));
});
...
const getPendingRequests = asyncHandler(async (req, res) => {
  ...
  const community = await Community.findOne({ communityName: communityName.toLowerCase() }).populate('pendingReq', 'username profilePicture');
...
const getJoinedCommunities = asyncHandler(async (req, res) => {
  ...
  return res.status(200).json(new ApiResponse(200, validCommunities, 'Joined communities fetched successfully'));
});
