...
interface FollowListDialogProps {
  ...
  username: string;
  /** When true, shows X button next to each user to remove them (for own profile). */
  allowRemove?: boolean;
  /** Called after a user is removed, so the parent can update counts. */
...
export default function FollowListDialog({
  ...
  const [confirmUser, setConfirmUser] = useState<FollowListUser | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchList = async () => {
      setLoading(true);
      try {
        const res = await api.get(`/user/${username}/follow-list?type=${type}`);
  ...
  const handleRemove = async () => {
    ...
    try {
      if (type === 'followers') {
        await api.post(`/remove-follower/${confirmUser._id}`);
        toast.success(`Removed ${confirmUser.username} from followers`);
      } else {
        await api.post(`/unfollow/${confirmUser._id}`);
        toast.success(`Unfollowed ${confirmUser.username}`);
      }
      setUsers((prev) => prev.filter((u) => u._id !== confirmUser._id));
      onRemove?.(confirmUser._id);
    } catch (error: any) {
                  ...
                  {allowRemove && (
                    <button
                      ...
                      className="p-1.5 rounded-full shrink-0
                                 text-text-muted-dark hover:text-danger hover:bg-danger/10
                                 [html.light_&]:text-text-muted-light [html.light_&]:hover:text-danger
      ...
      {confirmUser && (
        <>
          ...
          <div className="fixed z-[1001] top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2
                          w-[85vw] max-w-sm rounded-2xl p-6
                          bg-surface-dark [html.light_&]:bg-surface-card-light
                          border border-border-dark [html.light_&]:border-border-light
                          shadow-2xl text-center">
            <p className="text-sm text-text-dark [html.light_&]:text-text-light">
              Are you sure you want to {type === 'followers' ? 'remove' : 'unfollow'}{' '}
              <span className="font-bold">{confirmUser.username}</span>
              {type === 'followers' ? ' from your followers' : ''}?
            </p>
            <div className="mt-5 flex gap-3 justify-center">
              ...
              <button
                ...
                className="px-5 py-2 rounded-full text-sm font-semibold cursor-pointer
                           ...
                           shadow-lg shadow-danger/25 transition-all duration-200"
              >
