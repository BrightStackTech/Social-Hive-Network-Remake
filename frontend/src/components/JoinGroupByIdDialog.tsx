...
interface JoinGroupByIdDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

export default function JoinGroupByIdDialog({ open, onOpenChange }: JoinGroupByIdDialogProps) {
  const { api } = useAuth();
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>Join Group by ID</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-4 pt-2">
          ...
          <div className="relative group">
            ...
            <input
              ...
              placeholder="Paste the 24-character Group ID here..."
          ...
          <div className="min-h-[120px] flex flex-col items-center justify-center">
            {loading && (
              <div className="text-center py-6">
                ...
                <p className="mt-2 text-xs text-text-muted-dark">Searching for group...</p>
              </div>
            )}
            ...
            {!loading && !searched && !query.trim() && (
              <div className="text-center py-6 opacity-40">
                ...
                <p className="text-xs">Enter a group ID to find your destination</p>
              </div>
            )}

            {!loading && searched && isMember && (
              <div className="w-full p-4 rounded-2xl glass-dark [html.light_&]:bg-primary/5 border border-primary/20 animate-in fade-in slide-in-from-bottom-2 duration-300">
                <p className="text-sm font-medium text-text-dark [html.light_&]:text-text-light mb-4 text-center">
                  You're already a member of this group.
                </p>
                <button
                  onClick={() => {
                    navigate(`/groups/${query.trim()}`);
                    onOpenChange(false);
                  }}
            ...
            {!loading && searched && !result && !isMember && query.length >= 24 && (
              <div className="text-center py-6 text-danger/80">
                <p className="text-sm font-medium">No group found with this ID</p>
                ...
                <div className="mt-5">
                  {requested ? (
                    ...
                    >
                      ...
                      {joining ? 'Sending...' : 'Request to Join Group'}
                    </button>
                  )}
                </div>
              </div>
