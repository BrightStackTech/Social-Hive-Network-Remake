...
import { ThumbsUp, MessageCircle, Repeat2, Bookmark, ChevronLeft, ChevronRight, X, Share2 } from 'lucide-react';
import { formatDistanceToNow } from 'date-fns';
import toast from 'react-hot-toast';
import { Link, useNavigate } from 'react-router-dom';
import type { Post } from '../../components/posts/PostCard';
import { useAuth } from '../../context/AuthContext';
import { likePost, createRepost, savePost } from '../../api/index';
      ...
      while ((match = regex.exec(part)) !== null) {
        ...
        if (type === 'url') {
          ...
          if (matchCat) {
            ...
            // Always link categories, fallback to search if not found
            newParts.push(
...
export default function OthersPostCard({ post, followCallback }: Props) {
  const { user } = useAuth();
  ...
  return (
    <div className="rounded-2xl border border-border-dark [html.light_&]:border-gray-200 glass-dark [html.light_&]:bg-white [html.light_&]:shadow-sm p-4 flex flex-col gap-3 group/card hover:border-primary/30 transition-all duration-300">
        ...
        <div className="relative rounded-xl overflow-hidden bg-black/20 [html.light_&]:bg-gray-100 border border-border-dark [html.light_&]:border-gray-200">
          {isImage((post.media || [])[mediaIndex]) ? (
            <img
              ...
              alt="post media"
              className="w-full max-h-[450px] object-contain cursor-pointer transition-transform hover:scale-[1.01]"
      ...
      {/* Action bar */}
      <div className="flex items-center justify-between pt-1 border-t border-border-dark [html.light_&]:border-gray-100 mt-2">
